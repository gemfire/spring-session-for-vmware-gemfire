...
public class ClientServerProxyRegionSessionOperationsIntegrationTests extends AbstractGemFireIntegrationTests {
  ...
  @Autowired
  private GemfireTemplate sessionRegionTemplate;
  ...
  public void createReadUpdateExpireRecreateDeleteRecreateSessionResultsCorrectSessionCreatedEvents() {
    ...
    Awaitility.await().pollDelay(Duration.ofSeconds(MAX_INACTIVE_INTERVAL_IN_SECONDS + 1)).untilAsserted(() -> {
      ...
      event.ifPresent(abstractSessionEvent -> {
        ...
        assertThat(abstractSessionEvent.getSessionId()).isEqualTo(session.getId());
      });
    });
  ...
  @EnableGemFireHttpSession(poolName = "DEFAULT", maxInactiveIntervalInSeconds = MAX_INACTIVE_INTERVAL_IN_SECONDS)
  @SuppressWarnings("unused")
